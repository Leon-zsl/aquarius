...
void PacketSock::send_packet(const Packet &pck)
{
    pthread_mutex_lock(&_send_lock);
...
void PacketSock::dispatch_packet()
{
    if(!_running)
        return;
    ...
    if(_error != 0) {
        if(_handler != NULL)
            _handler->handle_error(this, _error);
        disconnect();
        return;
    }
    ...
    while(!_recv_queue.empty()) {
        ...
        if(_handler == NULL)
            continue;
        _handler->handle_packet(this, pck);
    }
    pthread_mutex_unlock(&_recv_lock);
}

void PacketSock::_recv_data()
{
    pthread_mutex_lock(&_recv_lock);
    if(!_running) return;
    ...
    while(true) {
        ...
        memcpy(_read_buf, _read_buf + _read_buf_pos, _read_buf_len - _read_buf_pos);
        _read_buf_pos = _read_buf_pos - len - 4;
    }
    pthread_mutex_unlock(&_recv_lock);
}

void PacketSock::_send_data()
{
    pthread_mutex_lock(&_send_lock);
    if(!_running) return;
    ...
    while(!_send_queue.empty()) {
        ...
        memcpy(_write_buf + _write_buf_pos + 4 + 4, data, pck.data_len());
        _write_buf_pos = _write_buf_pos + 4 + 4 + pck.data_len();
    }
    //pthread_mutex_unlock(&_send_lock);
